...
function investigatePrefix(allAccounts: BPAccount[], prefix: string) {
  ...
  return {
    ...
    leaf_summary: {
      ...
      net: Math.round(leafAccounts.reduce((s, a) => s + (a.mov_debit - a.mov_credit), 0)),
    },
    top_items: leafAccounts
      .filter(a => a.mov_debit > 0 || a.mov_credit > 0)
      .sort((a, b) => (b.mov_debit - b.mov_credit) - (a.mov_debit - a.mov_credit))
      .map(a => ({
        code: a.code,
        name: a.name,
        mov_debit: Math.round(a.mov_debit),
...
export async function GET() {
  try {
    const report = await fetchTestBalanceReport(2025, 12, 12);
    if (!report.file_url) {
      return NextResponse.json({ error: 'No file_url from Siigo for Dec 2025' }, { status: 500 });
    }

    const allAccounts = await parseBalanceExcel(report.file_url);

    return NextResponse.json({
      investigation: 'Diciembre 2025 — Arrendamientos (5120) + Gastos de Personal (5105)',
      arrendamientos_5120: investigatePrefix(allAccounts, '5120'),
      gastos_personal_5105: investigatePrefix(allAccounts, '5105'),
    });
  } catch (error: unknown) {
